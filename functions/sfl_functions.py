...
class loss_based_status():
    ...
    def record_loss(self, epoch, loss):
        if epoch <= self.warmup_epoch: # make sure your epoch starts from 1
            ...
            # 为什么要设置一个warmup_epoch？因为刚开始训练时有可能loss降得比较慢，这并不是因为模型训练好了。所以设置一个warmup_epoch。
        else:
            delta_loss = max(self.loss - loss, 0) # if loss reduces, this would be greater than 0 
            # delta_loss表示两次loss之间的差值，即两个epoch之间loss的变化量
        
        self.loss = loss
        # 如果loss的变化量小于某个阈值, 则将状态降级
        if delta_loss < self.threshold: # if delta loss is small, then we use replay tensor to update server （self.threshold==0时表示不适用降级策略，状态一直为A）
            if self.status == "A":
                self.status = "B"
            elif self.status == "B":
                self.status = "C"
