...
#         print(f"x的device:{x.device},idx_unshuffle的device:{idx_unshuffle.device}")
    ...
    def simco_loss(self, query: torch.Tensor, key: torch.Tensor, temperature=0.1,dt_m=10) -> torch.Tensor:
    ...
    def compute_simco(self, query, pkey, update_momentum = True, enqueue = True, tau = 0.99, pool = None, world_size = 1):
        ...
        if self.symmetric: # 使用互信息计算
            loss12, accu, q1, k2 = self.simco_loss(query, pkey, pool)
            loss21, accu, q2, k1 = self.simco_loss(pkey, query, pool)
            loss = loss12 + loss21
            pkey_out = torch.cat([k1, k2], dim = 0) # 将同一张图片的两个aug的keys都入队作为负例
        else:
            loss, accu, query_out, pkey_out = self.simco_loss(query, pkey, pool)
