...
def get_cifar10_dali_DDP(size_crops=None, nmb_crops=None, min_scale_crops=None, max_scale_crops=None, batch_size=16, num_workers=4, num_client = 1, data_proportion = 1.0, noniid_ratio =1.0, pairloader_option = "None", partition = 'noniid', partition_beta = 0.4, hetero = False, path_to_data = "../data"):
    # 先读取cifar10数据
    train_data, train_targets = load_cifar10(train=True, root=path_to_data)
    test_data, test_targets = load_cifar10(train=False, root=path_to_data)
    net_dataidx_map = {i:None for i in range(args.num_client)}
    # sampler_list = []
    # pipeline_list = []
    train_loader = []
