...
#                         sfl.log(f"epoch{epoch}  c_scheduler{client_index}:{sfl.c_lr_schedule[iteration]}")
...
#                     if args.cutlayer > 1:
                    for image_id in range(len(images)):
                        images[image_id] = images[image_id].to(device)
#                     if batch==1:
#                     pdb.set_trace()
...
#                         gradient_i = gradient_i.cpu()
            ...
            if loss_status.status == "A": # loss大于某个阈值的时候才更新client-side models
                ...
                #client backward
#                 pdb.set_trace()
                client_backward_swav(sfl, pool, gradient_dict, use_fp16 = args.use_fp16, scaler = scaler, use_swav_scheduler=args.use_swav_scheduler) # 各client以自己的gradient进行反向传播
            else:
        ...
        if not args.use_swav_scheduler:
...
#             sfl.c_instance_list[0].cpu()
        ...
        for client_indx in pool:
            train_loader[client_indx]._ever_consumed = False # 防止train_loader自动调用.reset()函数
