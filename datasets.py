    ...
    if pairloader_option != "None":
        if data_proportion > 0.0:
            ...
            # mem_loader也是一个包含所有client dataloader的list；
                if not dirichlet:
                    train_loader, net_data_counts = get_cifar10_pairloader(batch_size, num_workers, shuffle, num_client, data_proportion, noniid_ratio, pairloader_option, hetero, hetero_string, path_to_data, is_distributed)
                else:
        ...
        if not dirichlet:
            return train_loader, net_data_counts, mem_loader, test_loader
        else:
...
def get_cifar100(batch_size=16, num_workers=4, shuffle=True, num_client = 1, data_proportion = 1.0, noniid_ratio =1.0, augmentation_option = False, pairloader_option = "None", partition = 'noniid', aug_type = None, hetero = False, hetero_string = "0.2_0.8|16|0.8_0.2", path_to_data = "./data", is_distributed = False, dirichlet=False):
    if pairloader_option != "None":
        if not dirichlet:
            # MocoSFL的
            train_loader, net_data_counts = get_cifar100_pairloader(batch_size, num_workers, shuffle, num_client, data_proportion, noniid_ratio, pairloader_option, hetero, hetero_string, path_to_data, is_distributed)
        else:
#             Dirichlet分布的
...
def get_cifar100_pairloader(batch_size=16, num_workers=2, shuffle=True, num_client = 1, data_portion = 1.0, noniid_ratio = 1.0, pairloader_option = "None", hetero = False, hetero_string = "0.2_0.8|16|0.8_0.2", path_to_data = "./data", is_distributed=False):
    ...
    cifar100_training_loader, net_data_counts = get_multiclient_trainloader_list(train_data, num_client, shuffle, num_workers, batch_size, noniid_ratio, 100, hetero, hetero_string, is_distributed)
    
    return cifar100_training_loader, net_data_counts
...
def get_cifar10_pairloader(batch_size=16, num_workers=4, shuffle=True, num_client = 1, data_portion = 1.0, noniid_ratio = 1.0, pairloader_option = "None", hetero = False, hetero_string = "0.2_0.8|16|0.8_0.2", path_to_data = "./data", is_distributed=False):
    ...
    train_data = CIFAR10Pair(root=path_to_data, 
                             train=True, 
                             transform=train_transform, 
                             download=True) # train_data中的每条数据都是pair
    ...
    cifar10_training_loader, net_data_counts = get_multiclient_trainloader_list(train_data, num_client, shuffle, num_workers, batch_size, noniid_ratio, 10, hetero, hetero_string, is_distributed)
    
    return cifar10_training_loader, net_data_counts
...
def get_cifar10_trainloader(batch_size=16, num_workers=2, shuffle=True, num_client = 1, data_portion = 1.0, noniid_ratio = 1.0, augmentation_option = False, hetero = False, hetero_string = "0.2_0.8|16|0.8_0.2", path_to_data = "./data"):
    ...
    # 按照data_portion比例随机抽取原始CIFAR10训练集的一部分。data_portion默认为1.0。
