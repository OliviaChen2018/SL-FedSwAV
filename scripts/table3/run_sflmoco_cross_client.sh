...
# 这个sh文件是把一个table的所有实验都做了(所以才有这么大一个for循环)。包括dataset分别为cifar10, cifar100, imagenet12，每个dataset的情况下又包括不同的随机数种子seed，client的数量为100,200和1000的情况下，non_iid比例分别为0.2和1的情况。
